...
async (req, res) => {
    ...
    try {
        let tracks = await Utils.getPopulatedTracks();
        //console.log(tracks);
...
async (req, res) => {
    ...
    try {
        ...
        let deleteMedia = Promise.resolve();
        if(track.trackImage_id) deleteMedia.then(() => 
            Utils.deleteFileAsync(track.trackImage_id))
        if(track.location_id) deleteMedia.then(() => 
...
router.get("/playlists/:id/update",
async (req, res) => {
    let id = req.params.id;
    console.log(id);
    try{ 
        ...
        console.log("Playlist: " + playlist);
        playlist.userRef.passhash = undefined;
        const allTracks = await Track.getAll();

        res.json({playlist : playlist, allTracks: allTracks});
    }catch(err) {
        console.log(err.message);
        res.status(400).json({ err: err.message });
    }
...
async (req, res)=> {
    ...
    try{
        ...
        console.log("ERROR IN UPDATE PLAYLIST: " + err.message);
        res.status(400).json({ err: err.message });
    }
});
